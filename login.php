	...
	if($count == 1){
			...
			if (isset($_COOKIE["product_list"])) {
				...
				for ($i=0; $i < count($product_list); $i++) { 
					//After getting user id from database check user cart item if there is already product listed or not
					$verify_cart = "SELECT id FROM cart WHERE user_id = $_SESSION[uid] AND p_id = ".$product_list[$i];
					$result  = mysqli_query($con,$verify_cart);
					if(mysqli_num_rows($result) < 1){
						//if user is adding first time product into cart update user_id into database table with valid id
						$update_cart = "UPDATE cart SET user_id = '$_SESSION[uid]' WHERE ip_add = '$ip_add' AND user_id = -1";
						mysqli_query($con,$update_cart);
					}else{
						//if already that product is available into database table delete that record
				...
				//if user is logging from after cart page send cart_login
				echo "cart_login";
            ...
            //if user record is available in database then $count is equal to 1
            if($count == 1){
                $row = mysqli_fetch_array($run_query);
                $_SESSION["uid"] = $row["admin_id"];
                $_SESSION["name"] = $row["admin_name"];
                $ip_add = getenv("REMOTE_ADDR");
                //created a cookie in login_form.php page if that cookie is available means user is not login
